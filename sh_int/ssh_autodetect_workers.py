...
executer = concurrent.futures.ProcessPoolExecutor(max_workers=4)
...
def sshdetect(target, device, i):
    ...
    try:
        ...
        return target, best_match, timestamp, i, device
    ...
    try:
        for i, device in enumerate(target_devices, 1):
            ...
            if len(device.split(':')) == 2:
                ...
                target['host'] = host
                target['port'] = port
            # workers executer to detect device type
            future = executer.submit(sshdetect, target, device, i)
            futures.append(future)

        for future in concurrent.futures.as_completed(futures):
            target, best_match, timestamp, i, device = future.result()
            dct_inventory[i] = {
                "_id": i,
                "device": device,
                "target": target,
        ...
        logger.error('=== Detected ctrl+c KeyboardInterrupt ===')
        logger.error('=== Please wait until all running processes finish for the script to exit')
        print('=== Detected ctrl+c KeyboardInterrupt ===')
        print('=== Please wait until all running processes finish for the script to exit')
